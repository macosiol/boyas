...
void WiseRoute::initialize(int stage)
{
	...
	if(stage == 1) {
		...
		EV << "WiseRoute::initialize  host IP address =" << myNetwAddr << endl;
		EV << "WiseRoute::initialize  host macaddress=" << arp->getMacAddr(myNetwAddr) << endl;
...
void WiseRoute::handleSelfMsg(cMessage* msg)
{
	if (msg->getKind() == SEND_ROUTE_FLOOD_TIMER) {
		...
		EV << "UABC WiseRoute - handleSelfMessage: got unexpected message of kind " << msg->getKind() << endl;
		delete msg;
	}
}
...
void WiseRoute::handleUpperMsg(cMessage* msg)
{
    ...
    EV<< " UABC WISEROUTE UPPPPPPPPPPPPMSG >>>>>>>>>>>>>>>> " ;
	...
	if ( cInfo == NULL ) {
		...
		EV <<"UABCWiseRoute::handleUpperMsg WiseRoute: CInfo removed, netw addr="<< NetwControlInfo::getAddressFromControlInfo( cInfo ) <<endl;
	...
	EV<< " EL NOMBRE DEL PAQUETE ES " << pkt->getName();
	sendDown(pkt);
	nbDataPacketsSent++;
}
