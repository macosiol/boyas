...
void csma::handleUpperMsg(cMessage *msg) {
    ...
    // ------------ tomar el tipo de nodo de la capa de aplicación UABC---------------------- //
    EV << endl << " <<<<<<<<<< UABC TIPO PAQUETE >>>>>>>>>>>" << msg->getName() << endl;
    cModule* parentmod = getParentModule();
    cModule* applicationmod = parentmod->getParentModule()->getSubmodule("appl");
    MySensorApp* Applicationmod = dynamic_cast<MySensorApp*>(applicationmod);
    EV << endl << " <<<<<<<<<<< UABC TIPO DE NODO >>>>>>>>>>" << Applicationmod->gettipoNodo()  << endl;
    // ------------ tomar el tipo de nodo de la capa de aplicación UABC---------------------- //
...
void csma::handleLowerMsg(cMessage *msg) {
    ...
    if ((!(strcmp(msg->getName(), "HELLO_RESPONSE")))&&((Applicationmod->gettipoNodo()==1)||(Applicationmod->gettipoNodo()==2))) //si dos cadenas son iguales regresa 0
        {
          Applicationmod->settipoNodo(2);
          macPkt->setSrcAddr(LAddress::L2BROADCAST);
        }
    // ------------ cambiar a GATEWAY UABC---------------------- //
...
//				nbRxFrames++;
	...
	else if ((LAddress::isL2Broadcast(dest))&&((!(strcmp(msg->getName(), "HELLO")))||((Applicationmod->gettipoNodo()!=0)))) {
	    EV << "UABC >>>>>>>>>>>>>>>>>>>< packet BROADCAST...\n";
		executeMac(EV_BROADCAST_RECEIVED, macPkt);
	} else {
		debugEV << "packet not for me, deleting...\n";
		EV << "UABC >>>>>>>>>>>>>>>>>>>< packet not for me, deleting...\n";
		delete macPkt;
	}
}
